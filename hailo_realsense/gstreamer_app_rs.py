...
from gstreamer_helper_pipelines_rs import get_source_type
...
class GStreamerApp:
    ...
    def run(self):
        ...
        if not self.options_menu.disable_callback:
            ...
            depthsrc = self.pipeline.get_by_name("depthsrc")
            if identity is None:
                ...
                depthsrc_pad = depthsrc.get_static_pad("src")
                identity_pad.add_probe(Gst.PadProbeType.BUFFER, self.app_callback, self.user_data)
                depthsrc_pad.add_probe(Gst.PadProbeType.BUFFER, self.depth_callback, self.user_data)
        ...
        if self.source_type == "realsense":
            ...
            rs_thread = threading.Thread(target=realsense_thread, args=(self.pipeline, self.video_width, self.video_height, self.video_format))
            self.threads.append(rs_thread)
            rs_thread.start()
...
def realsense_thread(pipeline, video_width, video_height, video_format, hailo_context=None):
    print("RS thread starting")
    appsrc = pipeline.get_by_name("appsrc")
    appsrc.set_property("is-live", True)
    appsrc.set_property("format", Gst.Format.TIME)
    #pipeline.set_state(Gst.State.PLAYING)
    
    depthsrc = pipeline.get_by_name("depthsrc") 
    
    # Setup caps for appsrc
    appsrc.set_property(
        "caps",
        Gst.Caps.from_string(
    ...
    depthsrc.set_property(
        "caps",
        Gst.Caps.from_string(
            f"video/x-raw, format=GRAY16_LE, width={video_width}, height={video_height}, framerate=30/1"
        )
    )
    ...
    config.enable_stream(rs.stream.depth, video_width, video_height, rs.format.z16, 30)
    
    try:
       pipeline_rs.start(config)
    except Exception as e:
       print(f"Error in startting RS: {e}")
    
    frame_count = 0
    #buffer_duration = Gst.util_uint64_scale_int(1, Gst.SECOND, 30)
    print("RealSense process started")
    ...
    try:
        while True:
            ...
            # Wrap into Gst.Buffer
            buffer = Gst.Buffer.new_wrapped(frame.tobytes())
            buffer_duration = Gst.util_uint64_scale_int(1, Gst.SECOND, 30)
            buffer.pts = frame_count * buffer_duration
            buffer.duration = buffer_duration
            
            depth_buf = Gst.Buffer.new_wrapped(depth_data.tobytes())
            depth_buffer_duration = Gst.util_uint64_scale_int(1, Gst.SECOND, 30)
            depth_buf.pts = frame_count * depth_buffer_duration
            depth_buf.duration = depth_buffer_duration
            

            ret = appsrc.emit('push-buffer', buffer)
            ret_depth = depthsrc.emit('push-buffer', depth_buf)
            if ret != Gst.FlowReturn.OK:
                print("Failed to push buffer:", ret)
                break
        ...
        print(f"Error in RealSense thread: {e}")
